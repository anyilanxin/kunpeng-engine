...
class StringValueTest {
  ...
  @Test
  @DisplayName("writeJSON 转义引号/反斜杠/控制字符, 非 ASCII 原样保留")
  void writeJsonEscaping() {
    final StringBuilder builder = new StringBuilder();
    new StringValue("a\"b\\c\nd\te").writeJSON(builder);
    assertThat(builder.toString()).isEqualTo("\"a\\\"b\\\\c\\nd\\te\"");

    final StringBuilder control = new StringBuilder();
    new StringValue("x" + (char) 1 + "y").writeJSON(control);
    assertThat(control.toString()).isEqualTo("\"x\\u0001y\"");

    final StringBuilder unicode = new StringBuilder();
    new StringValue("鲲鹏").writeJSON(unicode);
    assertThat(unicode.toString()).isEqualTo("\"鲲鹏\"");
  }
}
