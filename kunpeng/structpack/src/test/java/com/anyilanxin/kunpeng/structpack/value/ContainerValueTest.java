...
class ContainerValueTest {
  ...
  @Test
  @DisplayName("字节视图值内容等值: 可作 set 元素与 map key")
  void byteViewEqualsInContainers() {
    final SetValue<BinaryValue> set = new SetValue<>(BinaryValue::new);
    set.add(element -> element.wrap(new UnsafeBuffer(new byte[] {1, 2, 3}), 0, 3));
    set.add(element -> element.wrap(new UnsafeBuffer(new byte[] {1, 2, 3}), 0, 3));
    set.add(element -> element.wrap(new UnsafeBuffer(new byte[] {9}), 0, 1));
    assertThat(set.size()).isEqualTo(2);

    final MapValue<BinaryValue, LongValue> map = new MapValue<>(BinaryValue::new, LongValue::new);
    map.put(new BinaryValue(new UnsafeBuffer(new byte[] {7, 7}), 0, 2), new LongValue(1));
    map.put(new BinaryValue(new UnsafeBuffer(new byte[] {7, 7}), 0, 2), new LongValue(2)); // 覆盖
    assertThat(map.size()).isEqualTo(1);
    assertThat(map.get(new BinaryValue(new UnsafeBuffer(new byte[] {7, 7}), 0, 2)).getValue())
        .isEqualTo(2);
  }

  @Test
  @DisplayName("容器内容等值: 数组保序/集合无序/map 无序")
  void containerEquals() {
    final ArrayValue<LongValue> a1 = new ArrayValue<>(LongValue::new);
    a1.add().setValue(1);
    a1.add().setValue(2);
    final ArrayValue<LongValue> a2 = new ArrayValue<>(LongValue::new);
    a2.add().setValue(1);
    a2.add().setValue(3);
    ...
    final SetValue<StringValue> s1 = new SetValue<>(StringValue::new);
    s1.add(element -> element.wrap("x"));
    s1.add(element -> element.wrap("y"));
    final SetValue<StringValue> s2 = new SetValue<>(StringValue::new);
    s2.add(element -> element.wrap("y"));
    s2.add(element -> element.wrap("x"));
    assertThat(s1).isEqualTo(s2);
    assertThat(s1).hasSameHashCodeAs(s2);

    final MapValue<StringValue, LongValue> m1 = new MapValue<>(StringValue::new, LongValue::new);
    m1.put(new StringValue("k"), new LongValue(7));
    final MapValue<StringValue, LongValue> m2 = new MapValue<>(StringValue::new, LongValue::new);
    m2.put(new StringValue("k"), new LongValue(7));
    assertThat(m1).isEqualTo(m2);
    assertThat(m1).hasSameHashCodeAs(m2);
  }

  @Test
  @DisplayName("消费式 put 重复 key 覆盖: 尺寸不变, 值更新, reset 可清空")
  void consumerPutDuplicateKey() {
    final MapValue<StringValue, LongValue> map = new MapValue<>(StringValue::new, LongValue::new);
    map.put(key -> key.wrap("dup"), value -> value.setValue(1));
    map.put(key -> key.wrap("dup"), value -> value.setValue(2));
    assertThat(map.size()).isEqualTo(1);
    assertThat(map.get(new StringValue("dup")).getValue()).isEqualTo(2);
    map.reset();
    assertThat(map.isEmpty()).isTrue();
  }
}
