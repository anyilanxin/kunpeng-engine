...
class BufferUtilTest {
  ...
  @Test
  @DisplayName("startsWith: 含 prefixOffset 区间与不匹配/过短分支")
  void startsWithRegions() {
    final byte[] prefix = {9, 9, 1, 2, 3};
    final byte[] content = {1, 2, 3, 4};
    assertThat(BufferUtil.startsWith(prefix, 2, 3, content, 0, 4)).isTrue();
    assertThat(BufferUtil.startsWith(prefix, 0, 5, content, 0, 4)).isFalse(); // 过短
    assertThat(BufferUtil.startsWith(prefix, 2, 3, content, 1, 3)).isFalse(); // 首字节不匹配
    assertThat(BufferUtil.startsWith(prefix, 3, 2, content, 1, 3)).isTrue(); // 子区间 {2,3}
  }
}
