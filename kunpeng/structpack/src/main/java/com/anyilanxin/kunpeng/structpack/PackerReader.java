...
public final class PackerReader {
  ...
  public long readVarInt() {
    ...
    while (true) {
      ...
      if (shift == 63 && (b & 0x7F) > 1) {
        // 第 10 字节仅允许载荷 0/1（其余位超出 64 位表示范围）, 非规范编码按损坏拒绝
        throw new StructPackException("varint 第 10 字节超出 64 位范围, offset=" + (offset - 1));
      }
  ...
  public void skipBytes(final int n) {
    if (n < 0) {
      // 负数来自长度 varint 截断到 int——ensureRemaining 对负数恒通过, 会令游标回退静默错读
      throw new StructPackException("跳过字节数为负: " + n + ", offset=" + offset);
    }
    ensureRemaining(n);
    offset += n;
  }

  /** 读一个长度/计数值并约束在 [0, 剩余字节] 内——每个 wire 元素至少占 1 字节, 超界即数据损坏 （负数来自 varint 到 int 的截断, 同样拒绝）。 */
  public int readBoundedVarInt(final String what) {
    final long value = readVarInt();
    if (value < 0 || value > remaining()) {
      throw new StructPackException(
          what + " 非法: " + value + "（剩余 " + remaining() + " 字节）, offset=" + offset);
    }
    return (int) value;
  }
  ...
  private int skipTyped(final int depth) {
    if (depth > MAX_TAG_DEPTH) {
      // 递归跳过无深度上限时, 损坏数据的深嵌套数组会以 StackOverflowError 逃逸
      throw new StructPackException("tag 嵌套深度超限: " + MAX_TAG_DEPTH);
    }
    ...
    switch (tag) {
      ...
      case TAG_BYTES, TAG_NESTED -> skipBytes(readBoundedVarInt("tag 值长度"));
      case TAG_ARRAY -> {
        final int count = readBoundedVarInt("tag 数组元素数");
