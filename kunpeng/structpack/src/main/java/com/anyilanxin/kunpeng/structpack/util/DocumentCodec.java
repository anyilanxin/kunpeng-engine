  ...
  private static final class Reader {
    ...
    Object readValue() {
      ...
      return switch (code) {
        ...
        case 0xc6 -> readBinary(readLength32()); // bin32
        case 0xd9 -> readString(readU8()); // str8
        case 0xda -> readString(readU16()); // str16
        case 0xdb -> readString(readLength32()); // str32
        case 0xdc -> readArray(readU16()); // array16
        case 0xdd -> readArray(readLength32()); // array32
        case 0xde -> readMap(readU16()); // map16
        case 0xdf -> readMap(readLength32()); // map32
    ...
    /** 32 位无符号长度: 超 int 上限早失败（负数长度会绕过 require 的边界检查） */
    int readLength32() {
      final long value = readU32();
      if (value > Integer.MAX_VALUE || value > end - position) {
        throw new IllegalArgumentException(
            "msgpack 32 位长度非法: " + value + "（剩余 " + (end - position) + " 字节）");
      }
      return (int) value;
    }
