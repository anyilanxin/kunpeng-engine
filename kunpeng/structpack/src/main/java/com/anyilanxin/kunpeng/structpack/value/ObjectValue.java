...
public class ObjectValue extends BaseValue {
  ...
  public ObjectValue declareProperty(final BaseProperty<? extends BaseValue> prop) {
    ...
    if (id < 1 || id > 127) {
      throw new StructPackException(
          prop.getKey(), "字段 id 非法（必须 1..127, 单字节 varint 保持 id 块规范形态）: " + id);
    }
  ...
  public void read(final PackerReader reader) {
    ...
    for (int i = 0; i < fieldCount; i++) {
      ...
      final int length = reader.readBoundedVarInt("字段值长度");
      if (slot > 0) {
        final int valueStart = reader.getOffset();
        declaredProperties.get(slot - 1).read(reader);
        final int consumed = reader.getOffset() - valueStart;
        if (consumed != length) {
          // id 不复用 + 类型不可变 ⇒ 合法数据必然一致; 不一致说明帧中段损坏, 早失败免得错位传播
          throw new StructPackException(
              getClass().getSimpleName()
                  + ": 字段 id "
                  + id
    ...
    for (final BaseProperty<? extends BaseValue> prop : declaredProperties) {
      if (!prop.hasValue()) {
        throw new StructPackException(String.format("属性 '%s' 没有有效值", prop.getKey()));
      }
    }
  }
  ...
  private void ensureWritable() {
    if (layoutFrozen) {
      ...
      for (int i = 0; i < props.length; i++) {
        if (!props[i].hasValue()) {
          throw new StructPackException(String.format("属性 '%s' 没有可写入的有效值", props[i].getKey()));
        }
      }
    } else {
      for (final BaseProperty<? extends BaseValue> prop : declaredProperties) {
        if (!prop.hasValue()) {
          throw new StructPackException(String.format("属性 '%s' 没有可写入的有效值", prop.getKey()));
        }
      }
    }
  }
