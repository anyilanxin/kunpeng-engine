...
public abstract class BaseProperty<T extends BaseValue> {
  ...
  protected T resolveValue() {
    if (isSet) {
      ...
      throw new StructPackException(key, "期望指定值或默认值，但两者皆未设置");
    }
  }
  ...
  public void write(final PackerWriter writer) {
    ...
    if (valueToWrite == null) {
      throw new StructPackException(key, "写入前期望已设置值或默认值，但两者皆未设置");
    }
    key.write(writer);
  ...
  @Override
  public int hashCode() {
    return Objects.hash(id, getKey(), resolveValueOrNull());
  }
