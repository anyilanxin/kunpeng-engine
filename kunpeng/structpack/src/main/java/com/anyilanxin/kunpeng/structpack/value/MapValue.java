...
public final class MapValue<K extends BaseValue, V extends BaseValue> extends BaseValue
  ...
  /** 追加一对键值槽位（同 key 覆盖旧值, 重复的 key 槽位回收进池） */
  public void put(final K key, final V value) {
    final int index = indexOfKey(keys, key);
    if (index >= 0) {
      final V old = values.set(index, value);
      if (old != null) {
        old.reset();
        valuePool.add(old);
      }
  ...
  private K newKey() {
    return keyPool.isEmpty() ? keyFactory.get() : keyPool.removeLast();
  }

  private V newValue() {
    return valuePool.isEmpty() ? valueFactory.get() : valuePool.removeLast();
  }

  @Override
  public void read(final PackerReader reader) {
    reset();
    final int size = reader.readBoundedVarInt("map 元素数");
  ...
  public boolean equals(final Object o) {
    ...
    for (int i = 0; i < keys.size(); i++) {
      final int index = indexOfKey(that.keys, keys.get(i));
      if (index < 0 || !values.get(i).equals(that.values.get(index))) {
        return false;
      }
    }
    return true;
  ...
  public int hashCode() {
    int h = 0;
    for (int i = 0; i < keys.size(); i++) {
      h += keys.get(i).hashCode() * 31 + values.get(i).hashCode();
    }
    return h;
  }

  private static <K extends BaseValue> int indexOfKey(final List<K> keys, final BaseValue key) {
    for (int i = 0; i < keys.size(); i++) {
      if (keys.get(i).equals(key)) {
        return i;
      }
