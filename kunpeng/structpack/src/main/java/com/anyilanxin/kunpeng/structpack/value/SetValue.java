...
public final class SetValue<T extends BaseValue> extends BaseValue implements Iterable<T> {
  ...
  public SetValue<T> add(final Consumer<T> value) {
    final T element = pool.isEmpty() ? valueFactory.get() : pool.removeLast();
  ...
  public boolean equals(final Object o) {
    ...
    for (final T value : values) {
      if (!containsValue(that.values, value)) {
        return false;
      }
    }
    return true;
  ...
  private static <T extends BaseValue> boolean containsValue(
      final List<T> values, final BaseValue value) {
  ...
  public void read(final PackerReader reader) {
    reset();
    final int size = reader.readBoundedVarInt("set 元素数");
