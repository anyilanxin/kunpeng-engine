...
public class BusinessDispatchChangePartitionProcessor extends AbstractBusinessDispatchProcessor {
  ...
  public BusinessDispatchChangePartitionProcessor(final LogEventWriter writer) {
    super(writer);
    this.writer = writer;
  ...
  public void processRecord(final LogRecord<BusinessDispatchPlanRecord> record) {
    ...
    int replicationFactor = partitioning.getReplicationFactor();
    final BusinessClusterMetaRecord oldClusterMeta = repositoryBusiness.getClusterMeta();
    if (!value.isInitialize() && oldClusterMeta != null) {
      replicationFactor = oldClusterMeta.getReplicationFactor();
      final List<PartitionInfoMetaRecord> meta = oldClusterMeta.getMetaRecord();
      value.setOldMeta(meta);
      value.setOldPartitionsCount(oldClusterMeta.getCurrentPartitionCount());
    } else {
      value.setOldPartitionsCount(0);
    }
    value.setOldReplicationFactor(replicationFactor);
    value.setExpectReplicationFactor(replicationFactor);
    writer.addCommand(
  ...
  public BusinessDispatchPlanLifeCycle valueLifeCycle() {
    return BusinessDispatchPlanLifeCycle.CHANGE_PARTITION;
  }
}
