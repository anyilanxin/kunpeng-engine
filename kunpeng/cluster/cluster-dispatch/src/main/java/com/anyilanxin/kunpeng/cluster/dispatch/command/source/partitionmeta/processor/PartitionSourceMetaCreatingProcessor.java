...
/**
 * 分区 source 元数据创建命令处理器。
 *
 * @author zxuanhong
 * @since 2026.9.0
 */
public class PartitionSourceMetaCreatingProcessor extends AbstractPartitionSourceMetaProcessor {
  protected final LogEventWriter writer;

  public PartitionSourceMetaCreatingProcessor(final LogEventWriter writer) {
    super(writer);
    this.writer = writer;
  }
  ...
  public void processRecord(final AdminLogRecord<PartitionSourceMetaRecord> record) {
    ...
    writer.addEvent(-1, PartitionSourceMetaLifeCycle.CREATED, record.getRequestId(), value);
  }

  @Override
  public PartitionSourceMetaLifeCycle valueLifeCycle() {
    return PartitionSourceMetaLifeCycle.CREATING;
  }
}
