...
public class BusinessDispatchClusterPlanCreatingProcessor
  ...
  public BusinessDispatchClusterPlanCreatingProcessor(final LogEventWriter writer) {
    super(writer);
    this.writer = writer;
  ...
  public void processRecord(final LogRecord<BusinessDispatchPlanRecord> record) {
    ...
    if (value.getExpectReplicationFactor() > memberIds.size()) {
      LOGGER.info(
          "\n----当前业务端需要集群成员不足，需要进行延迟调度-----期望成员数:{},实际成员数：{}\n",
          value.getExpectReplicationFactor(),
          memberIds.size());
      delayedRecord.reset();
      delayedRecord
          .setDelayedId(writer.nextKey())
          .setDelayedType(DelayedType.BUSINESS_PLAN)
      ...
      writer.addEvent(
          ...
          BusinessDispatchPlanLifeCycle.PLAN_DELAYED,
          record.getRequestId(),
          value);
      delayChecker.schedule(delayedRecord.getDueDate());
      return;
    }
    // 满足条件，进行计划制定（结果直接补全到 value 的 meta 与 executionPlan）
    planGenerator.createPlan(value, memberIds);
    writer.addEvent(
        value.getDispatchPlanId(),
        BusinessDispatchPlanLifeCycle.PLAN_CREATED,
        record.getRequestId(),
        value);
    if (value.isApplyPlan()) {
      writer.addCommand(
          value.getDispatchPlanId(),
          BusinessDispatchPlanLifeCycle.EXECUTING,
          record.getRequestId(),
          value);
    }
  }
  ...
  public BusinessDispatchPlanLifeCycle valueLifeCycle() {
    return BusinessDispatchPlanLifeCycle.PLAN_CREATING;
  }
}
