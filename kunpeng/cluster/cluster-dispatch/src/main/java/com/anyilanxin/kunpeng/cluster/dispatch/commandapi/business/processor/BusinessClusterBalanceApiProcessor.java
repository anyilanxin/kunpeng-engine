...
import com.anyilanxin.kunpeng.protocol.admin.impl.record.commandapi.business.dispatch.BusinessClusterBalanceRequestRecord;
 ...
 * @since 2026.9.0
 */
public class BusinessClusterBalanceApiProcessor
    extends AbstractBusinessApiProcessor<BusinessClusterBalanceRequestRecord> {
  ...
  public BusinessClusterBalanceApiProcessor(final LogEventWriter writer) {
    this.writer = writer;
  ...
  @Override
  public void processRecord(final AdminLogRecord<BusinessClusterBalanceRequestRecord> record) {
    final BusinessClusterBalanceRequestRecord value = record.getValue();
    ...
    final int currentReplicationFactor = clusterMeta.getCurrentReplicationFactor();
    final Set<Member> members = membershipService.getMembers(ZoneType.BROKER.getType());
    if (members.size() < currentReplicationFactor) {
      writer.adErrorResponse(
          record.getRequestId(),
          -1,
          "集群节点数量不足，实际:" + members.size() + ",需要最低节点数量:" + currentReplicationFactor);
      return;
    }
    planRecord.reset();
    planRecord
        .setApplyPlan(value.isApplyPlan())
        .setDispatchPlanType(BusinessDispatchType.CLUSTER_BALANCE);
    writer.addCommand(
        -1, BusinessDispatchPlanLifeCycle.CLUSTER_BALANCE, record.getRequestId(), planRecord);
  }

  @Override
  public BusinessDispatchApiValueLifeCycle valueLifeCycle() {
    return BusinessDispatchApiValueLifeCycle.CLUSTER_BALANCE_REQUEST;
  }
}
