 ...
 * @since
 */
public abstract class AbstractAdminDispatchExecutionProcessor
    implements LogEventProcessorSingleState<AdminDispatchPlanExecutionRecord> {
  ...
  public AbstractAdminDispatchExecutionProcessor(final LogEventWriter writer) {
    this.writer = writer;
  ...
  @Override
  public abstract AdminDispatchPlanExecutionLifeCycle valueLifeCycle();
  ...
  protected void scheduleExecutionRetry(
      final long requestId, final AdminDispatchPlanExecutionRecord execution) {
    delayedRecord.reset();
    delayedRecord
        .setDelayedId(writer.nextKey())
        .setDelayedType(DelayedType.ADMIN_EXECUTION)
