...
import com.anyilanxin.kunpeng.cluster.cluster.messaging.impl.HeartbeatWireFormat.Setup;
import com.anyilanxin.kunpeng.cluster.cluster.messaging.impl.HeartbeatWireFormat.SetupAck;
 ...
 * 连接握手阶段的心跳协商处理器。
 *
 * <p>心跳启用与否、心跳是否携带负载帧，都由双方在握手时一次性协商完成：客户端发起协商帧 （携带期望超时与本端负载能力），服务端以"双方都支持负载"这一 AND 语义裁决后回应答帧，
 * 随后两侧各自安装配置一致的 {@link HeartbeatHandler} 并把本处理器移出管道——因此最终两侧 行为总是对称的，不存在一侧带负载、另一侧不带的情况。
 *
...
public abstract sealed class HeartbeatSetupHandler extends ChannelDuplexHandler {
  ...
  private static void installHeartbeatHandler(
      ...
      final HeartbeatHandler negotiated,
      final String afterHandler) {
    ctx.pipeline().addAfter(afterHandler, HeartbeatHandler.HEARTBEAT_HANDLER_NAME, negotiated);
  }
