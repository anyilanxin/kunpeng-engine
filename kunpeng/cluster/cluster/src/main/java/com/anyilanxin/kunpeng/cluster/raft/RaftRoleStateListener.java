...
/**
 * 业务侧粗粒度分区状态监听器：把 Raft 角色（{@link RaftRoleChangeListener}）与快照复制事件 （{@link
 * SnapshotReplicationListener}）聚合为 LEADER / FOLLOWER / INACTIVE 三态视图。
 *
 ...
 * #onInactive(long)}。快照复制开始时确定触发 {@link #onInactive(long)}（日志将被重置、消费者需 关闭，业务视角不可用）；快照复制结束时确定触发
