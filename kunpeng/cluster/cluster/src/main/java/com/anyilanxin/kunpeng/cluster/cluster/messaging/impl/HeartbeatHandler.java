 ...
 * 为已有连接叠加双向心跳能力的处理基类。
 *
 * <p>两侧管道的第一个处理器都必须是参数一致的 {@link IdleStateHandler}（超时配置不同会造成 误判），随后客户端侧挂 {@link Client}、服务端侧挂
 * {@link Server}。
 *
 * <p>交互约定：
 *
 * <ul>
 *   <li>心跳由客户端发起：写空闲超过阈值时（IdleStateHandler 发出 WRITER_IDLE 事件）发送一条 心跳请求；
 *   <li>服务端收到心跳请求（{@link ProtocolRequest} 形态）后立即回复一条心跳响应（{@link ProtocolReply} 形态）；
 *   <li>客户端读空闲超时仍未等到任何报文时关闭连接；
 *   <li>服务端读空闲超时时，只有确认收到过至少一次心跳才关闭连接——这样可以兼容根本不发心跳 的旧版本客户端，不至于过早掐断连接。
 * </ul>
 */
abstract sealed class HeartbeatHandler extends ChannelDuplexHandler {
  ...
  static final class Client extends HeartbeatHandler {
     ...
     * 清理已兑现的心跳。
     *
     * <p>任意 ProtocolReply（不只心跳应答）都证明信道仍在收发，因此把 id 不大于该应答的未决 心跳一并移除；若只移除最新一个，漏发的心跳会持续堆积，直到连接关闭才释放内存。
     */
