 ...
 *
 * <ul>
 *   <li>心跳探测（PING）：{@code sentAtMillis 8B}
 *   <li>探测应答（PONG）：{@code receivedAtMillis 8B}
 *   <li>心跳协商（SETUP）：{@code timeoutMillis 8B + wantsPayload 1B}
 *   <li>协商应答（SETUP_ACK）：{@code heartbeatEnabled 1B + sendPayload 1B}
 * </ul>
 *
 * <p>请求/应答的关联 id 由外层 ProtocolRequest/ProtocolReply 信封承载，帧内不重复。解码失败 （魔数/版本/长度/类型不符）一律返回 {@code
 * null}，调用方按对端不支持心跳降级处理。
 */
final class HeartbeatWireFormat {
  ...
  static @Nullable SetupAck decodeSetupAck(final byte[] frame) {
    if (frame == null || frame.length != SETUP_ACK_FRAME_BYTES || kindOf(frame) != KIND_SETUP_ACK) {
      return null;
    }
    final var buffer = ByteBuffer.wrap(frame);
    return new SetupAck(buffer.get(PREFIX_BYTES) != 0, buffer.get(PREFIX_BYTES + Byte.BYTES) != 0);
  }
