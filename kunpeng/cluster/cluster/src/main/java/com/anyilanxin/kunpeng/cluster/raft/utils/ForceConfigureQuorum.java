 ...
 * 成员确认即判定成功，掉线或拒绝的少数派不阻塞强制变更；一旦"已赞成 + 剩余未决"不可能达到 多数（如过半成员不可达）则立即判定失败，避免整轮悬挂。被强制丢弃的成员随后在 poll/vote
 * 阶段会被按新配置拒绝。
 *
 * <pre>
...
public final class ForceConfigureQuorum {
  ...
  /** 记录来自 {@code member} 的拒绝/不可达。少数派失败不阻塞；当已赞成与剩余未决之和不再可能 达到多数时，立即判定整轮失败。 */
