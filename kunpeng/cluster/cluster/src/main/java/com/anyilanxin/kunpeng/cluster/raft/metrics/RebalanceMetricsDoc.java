...
public enum RebalanceMetricsDoc implements CustomMeterDocumentation {
  ...
  PARTITION_PAUSE_DURATION(
      "zeebe_cluster_rebalance_partition_pause_duration",
      "Duration a partition was paused during a leadership transfer",
      Type.TIMER),
  /** 分区是否因 Leader 转移而暂停（1=暂停中，0=否） */
  PARTITION_PAUSED(
      "zeebe_cluster_rebalance_partition_paused",
      "1 while a partition is paused for a leadership transfer, 0 otherwise",
      Type.GAUGE),
  /** 单次协作式 Leader 转移的耗时（按结果打标签） */
  PARTITION_TRANSFER_DURATION(
      "zeebe_cluster_rebalance_partition_duration",
      "Duration of a per-partition coordinated leadership transfer attempt, by result",
      Type.TIMER);
