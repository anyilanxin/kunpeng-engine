...
public interface JournalRecordSerializer {
   ...
   * @return either a {@link BufferOverflowException} when the record does not fit, or the number of
   *     bytes written
   */
  default Either<BufferOverflowException, Integer> writeData(
      final JournalRecordData record, final MutableDirectBuffer buffer, final int offset) {
    return writeData(
   ...
   * com.anyilanxin.kunpeng.cluster.raft.journal.CorruptedJournalException} otherwise.
   */
