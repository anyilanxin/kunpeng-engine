 ...
 *
 * <p>按快照边界截断旧日志的思想参考自 Apache-2.0 的 SOFAJRaft LogManager（日志清理）， 典型时序：
 *
 * <pre>
...
public final class LogCompactor {
  ...
  /** 在可截断边界之外额外保留的条目数，让落后的跟随者仍能靠条目复制追上， 而不是被迫走完整的快照传输。 */
  ...
  /**
   * 记录允许截断到的最高索引，随后的 {@link #compact()} 或 {@link #compactIgnoringReplicationThreshold()} 会删到该位置为止。
   *
   * <p>注意：可在任意线程调用。
   */
  @VisibleForTesting
  ...
  /**
   * 截到最新可截断边界，但保留一个复制阈值宽度的条目，方便轻微落后的跟随者低成本追赶。 由于快照是异步完成的、边界也随之推进，完全可能本次什么都没删 （例如上次运行后没有新快照）。
   *
   * @return 是否实际删除了数据
   */
  ...
  public void compactFromSnapshots(final FileSnapshotStore snapshotStore) {
    snapshotStore
        .getCompactionBound()
        .toCompletableFuture()
