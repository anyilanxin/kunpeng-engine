...
final class RaftMemberContextTest {
  ...
  @Test
  void closeShutsTheSnapshotChunkBatcher() {
    final var context = freshContext();
    final var chunkReader = mock(SnapshotChunkReader.class);
    context.setSnapshotChunkBatcher(new SnapshotChunkBatcher(chunkReader));
  ...
  /**
   * 成员类型变更（如 PASSIVE 晋升 ACTIVE）会在追加在途时调用 resetState 清零在途计数；
   * 迟到响应的 completeAppend 不得把计数推到负数，否则 canAppend/canHeartbeat 永久为
   * false，领导者静默停止向该成员复制日志（仅 ConfigureRequest 仍能发出）。
   */
  @Test
  void lateAppendCompletionAfterResetStateKeepsReplicationEligible() {
    final var context = freshContext();
    context.startAppend();
