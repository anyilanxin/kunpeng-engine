...
public class KunpengTestNode {
  ...
  public KunpengTestNode(final int id, final File directory, final MeterRegistry meterRegistry) {
  ...
  public CompletableFuture<Void> start(final Collection<KunpengTestNode> nodes) {
    actorScheduler = ActorScheduler.newDefaultActorScheduler();
    actorScheduler.start();
    cluster = buildCluster(nodes);
    final Set<MemberId> members =
        nodes.stream().map(KunpengTestNode::getMember).map(Member::id).collect(Collectors.toSet());
  ...
  private AtomixCluster buildCluster(final Collection<KunpengTestNode> nodes) {
  ...
  private NodeDiscoveryProvider buildDiscoveryProvider(final Collection<KunpengTestNode> nodes) {
    return BootstrapDiscoveryProvider.builder()
        .withNodes(nodes.stream().map(KunpengTestNode::getNode).collect(Collectors.toList()))
        .build();
  }
  ...
  public String toString() {
    return "KunpengTestNode{" + "member=" + member + '}';
  }
}
