...
public class KunpengIT {
  ...
  @Parameter(1)
  public Collection<Function<TemporaryFolder, KunpengTestNode>> nodeSuppliers;

  private final TestAppender appenderWrapper = new TestAppender();

  private Collection<KunpengTestNode> nodes;
  ...
  private static Function<TemporaryFolder, KunpengTestNode> provideNode(final int id) {
    return tmp -> new KunpengTestNode(id, newFolderUnchecked(tmp, id), meterRegistry);
  }
  ...
  public void shouldAppendAndReplicate() {
    ...
    final LogAppender appender = helper.awaitLeaderAppender(partitionId);
  ...
  public void shouldFailover() {
    ...
    // given
    final int partitionId = 1;
    final KunpengTestNode originalLeader = helper.awaitLeader(partitionId);
    final Collection<KunpengTestNode> followers = new ArrayList<>(nodes);
    followers.remove(originalLeader);

    // when
    originalLeader.stop().join();
    final KunpengTestNode newLeader = helper.awaitLeader(partitionId, followers);
  ...
  public void shouldAppendAllEntriesEvenWithFollowerFailures() {
    ...
    // given
    final int partitionId = 1;
    final KunpengTestNode leader = helper.awaitLeader(partitionId);
    final LogAppender appender = helper.awaitLeaderAppender(partitionId);
    final List<KunpengTestNode> followers =
        nodes.stream().filter(node -> !node.equals(leader)).toList();
    final List<IndexedRaftLogEntry> entries = new ArrayList<>();

    // when
    for (int i = 0; i < followers.size(); i++) {
      final KunpengTestNode follower = followers.get(i);
      final List<KunpengTestNode> others =
  ...
  public void shouldNotifyCommitListeners() {
    ...
    final int partitionId = 1;
    final LogAppender appender = helper.awaitLeaderAppender(partitionId);
    final Map<KunpengTestNode, CommitListener> listeners =
        nodes.stream()
            .collect(
                Collectors.toMap(
                    Function.identity(),
    ...
    for (int i = 0; i < 5; i++) {
      ...
      for (final KunpengTestNode node : nodes) {
  ...
  private Collection<KunpengTestNode> buildNodes() {
  ...
  private void stop() throws InterruptedException, ExecutionException, TimeoutException {
    CompletableFuture.allOf(
            nodes.stream().map(KunpengTestNode::stop).toArray(CompletableFuture[]::new))
        .get(30, TimeUnit.SECONDS);
  }
