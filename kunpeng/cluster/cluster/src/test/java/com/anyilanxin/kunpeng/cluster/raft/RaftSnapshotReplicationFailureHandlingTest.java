...
import com.anyilanxin.kunpeng.cluster.raft.protocol.TestRaftServerProtocol.ResponseInterceptor;
...
public class RaftSnapshotReplicationFailureHandlingTest {
  ...
  public void shouldCompleteSnapshotReplicationDespiteHeartbeats() throws Throwable {
    // given
    final int numberOfChunks = 10;
    // 每分片 4MiB、共 10 个传输批（每批恰好一个整文件分片）
    disconnectFollowerAndTakeSnapshot(numberOfChunks, 4 * 1024 * 1024);
  ...
  private void disconnectFollowerAndTakeSnapshot(final int numberOfChunks, final int chunkSize)
      throws Exception {
    follower = raftRule.getFollower().orElseThrow();
    raftRule.partition(follower);
    ...
    raftRule.takeSnapshot(leader, commitIndex, numberOfChunks, chunkSize);
    raftRule.appendEntry();
  }
