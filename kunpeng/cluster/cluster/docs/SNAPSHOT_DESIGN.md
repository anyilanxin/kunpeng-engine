...
- **接收与拍摄统一为 `PersistableSnapshot`**：不再有独立的 `ConstructableSnapshot` 类型。
...
|---|---|---|
| 源分区 leader | 注册**分片提供**处理器（逐片应答） | 服务端**推送循环**（逐片发给目标） |
| 目标分区 leader | 服务端**拉取循环**（actor 逐片请求） | 注册**分片接收**处理器（逐片收 → Appender） |
| 触发方 | 新分区启动 | 源分区删除前 |

- **处理器注册**仍在 `RaftPartitionServer` 的角色生命周期内（leader 注册/离开卸载）；
- **发起入口**（`getLatestSnapshot`/`pushMergeSnapshot`）在业务持有的
  `SnapshotRemoteTransferService` 上——服务是无状态客户端，可在任意节点发起；
- 连接层面：`ChannelPool` 按（地址, subject）建独立 TCP 连接——传输/推送各 subject 与 raft
  消息的连接物理隔离，无队头阻塞。

### 6.4 写入侧批量缓冲（Appender 内部）

传输协议逐片，但**落盘仍批量**：`SnapshotChunkAppender` 缓冲到达的分片，累计字节 ≥
**maxWriteBatchBytes（缺省 1MiB）** 时整批写盘（按文件分组、偏移定位，**每文件每批一次
force**）；单片超限独占一批；`flush()` 清尾批——协议简单（一问一答）与 IO 效率（批量落盘）
解耦。
...
public interface SnapshotChunk {
  default SnapshotType getType();   // 缺省常规
  String getSnapshotId();           // 唯一快照标识
  String getChunkName();            // 分片名（文件名@字节偏移；同名 = 同一大文件的多片）
...
分片规则：**单个文件太大 → 按分片尺寸切多片（chunkName 同名不同偏移）；
单个文件太小 → 一个 chunk 携带整个文件**（不分片）。

### 6.2 快照传输服务（模块对外门面，业务持有）

传输能力收敛为快照模块的一个**服务**（`SnapshotRemoteTransferService`），由业务侧持有，
不挂在 RaftPartitionServer 上；**传输单元 = 单个 `SnapshotChunk`**，actor 循环驱动：

```java
public final class SnapshotTransferService {

  /** 拉取：向指定分区请求最新快照，逐片拉取直到完成；返回持久化后的快照。 */
  ActorFuture<@Nullable PersistedSnapshot> getLatestSnapshot(final PartitionId partitionId);

  /** 推送：把本分区最新的合并快照推到目标分区 leader。 */
  ActorFuture<Void> pushMergeSnapshot(final PartitionId targetPartitionId);
}
```
...
```java
final var transferService = new SnapshotTransferService(
    membershipService,        // 拓扑解析（分区角色经成员属性广播）
    communicationService,     // 跨节点通信
    persistedSnapshotStore);  // 本地快照存储（拉取接收 / 推送读取）
```

拉取流程：

```
getLatestSnapshot(分区Id)
  → 拓扑解析目标分区 leader
  → 循环（actor 驱动，一问一答）：
      请求(续传位置=上个分片名) ──→ 指定分区收到请求
                                  → 定位快照 → 返回下一个 SnapshotChunk
      ←── SnapshotChunk（非 null）：
          走 pending 逻辑（Appender 校验+缓冲写入）→ 继续循环
      ←── null：传输完成
  → flush + 校验 manifest → 落盘 → 原子移动到正式快照 → 返回持久快照信息
```

推送流程（merge，方向相反）：源分区 leader 主动逐片推送到目标分区 leader 的接收
处理器；协议对称（每请求一个 chunk，结束标志收尾），复用同一套分片定位与 pending 写入。

要点：
- **逐 chunk 请求-应答**，每次一个分片；`getChunkName` 相同 = 同一大文件的多片
- 返回 **null 表示传输完成**（无批量帧、无 hasMore 标志）
- 分片规则：单文件 ≤ 分片尺寸 → 一个 chunk 携带整文件；> 分片尺寸 → 切多片
- 断点续传：请求携带上个分片名，按偏移定位继续
- 失败即中止，可整体重发（目标侧按快照 id 建会话）
...
- 接口形态改造：类型化查询（`getLatestSnapshot`→`Optional<RaftSnapshot>` 等）、
  ...
  `copyForBootstrap`、接收统一为 `PersistableSnapshot`（取消 `TransientSnapshot`/`ConstructableSnapshot`）
...
- ~~SnapshotTransferService 门面~~ → **已实现**：业务持有的快照模块服务（构造吃
  membership/communication/本地存储/超时）；`getLatestSnapshot(PartitionId)`（缺省 BOOTSTRAP，
  可按类型）与 `pushMergeSnapshot(PartitionId)` 均返回 `ActorFuture`；分区侧仅保留处理器注册
- ~~传输协议改逐 chunk~~ → **已实现**：`SnapshotChunkResponse`（一个分片或 null 完成）/
  `SnapshotPushRequest`（一个分片或结束标志）；批量帧与 `fillBatch` 已删；`getTotalCount`
  已从接口/线格式/校验逻辑全部移除（完整性由 manifest 逐文件 CRC 承担）
