...
final class RaftClusterContextTest {
  ...
  private static Configuration forcedConfiguration(
      final long index, final List<RaftMember> members) {
    return new Configuration(index, TERM, Instant.now().toEpochMilli(), members, List.of(), true);
  }
  ...
  void forcedConfigurationPrunesContextsImmediately() {
    ...
    // 强制配置：立即生效，不等提交
    context.configure(forcedConfiguration(2, List.of(local)));
