...
final class StaticClusterMembershipService implements ClusterMembershipService {
  ...
  public Set<Member> getMembers(final String zone) {
    ...
    return getMembers().stream()
        .filter(member -> member.zone() != null && currentZone.equalsIgnoreCase(member.zone()))
        .collect(Collectors.toUnmodifiableSet());
  }

  @Override
  public Set<MemberId> getMemberIds(final String zone) {
    return getMembers(zone).stream().map(Member::id).collect(Collectors.toUnmodifiableSet());
  }
