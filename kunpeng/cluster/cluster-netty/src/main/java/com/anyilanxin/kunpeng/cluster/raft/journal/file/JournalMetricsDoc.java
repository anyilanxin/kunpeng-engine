...
public enum JournalMetricsDoc implements CustomMeterDocumentation {
  ...
  SEGMENT_CREATION_TIME(
      "atomix_segment_creation_time", "Time spent creating a new segment", Type.TIMER),
  /** 截断 segment 的耗时 */
  SEGMENT_TRUNCATE_TIME(
      "atomix_segment_truncate_time", "Time spent truncating a segment", Type.TIMER),
  /** segment 落盘的耗时 */
  SEGMENT_FLUSH_TIME(
      "atomix_segment_flush_time", "Time spent flushing a segment to disk", Type.TIMER),
  /** 全部脏 segment 落盘的耗时 */
  JOURNAL_FLUSH_TIME(
      "atomix_journal_flush_time", "Time spent flushing all dirty segments", Type.TIMER),
  /** segment 数量 */
  ...
  SEGMENT_ALLOCATION_TIME(
      "atomix_segment_allocation_time", "Time spent allocating a new segment", Type.TIMER),
  /** 追加到 journal 的数据速率（KiB） */
  APPEND_DATA_RATE(
      "atomix_journal_append_data_rate",
      "The rate in KiB at which data is appended to the journal",
      Type.COUNTER),
  /** 追加到 journal 的条目速率（按条数） */
  APPEND_RATE(
      "atomix_journal_append_rate",
      "The rate at which entries are appended to the journal, by entry count",
      Type.COUNTER),
  /** 追加 journal 记录的耗时分布（不含落盘） */
  APPEND_LATENCY(
      "atomix_journal_append_latency",
      "Distribution of time spent appending journal records, excluding flushing",
      Type.TIMER),
  /** 定位到指定索引的耗时分布 */
  SEEK_LATENCY(
      "atomix_journal_seek_latency",
      "Distribution of time spent seeking to a specific index",
      Type.TIMER),
  /** journal 磁盘占用（segment 文件总大小，字节） */
  JOURNAL_SIZE_BYTES(
      "atomix_journal_size_bytes",
      "Total size of the journal segment files on disk in bytes",
      Type.GAUGE);
