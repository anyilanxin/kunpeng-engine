...
public class SwimMembershipProtocolConfig extends GroupMembershipProtocolConfig {
  ...
  private static final int DEFAULT_METADATA_ANTI_ENTROPY_INTERVAL = 300_000;
  private static final int DEFAULT_METADATA_PULL_JITTER = 500;
  ...
  /**
   * 随 gossip/sync 报文内联传播的属性名白名单（如事件订阅表这类广播寻址必需的小属性）。 其余属性只随 metadataVersion 触发的拉取通道传播，大幅缩减稳态报文体积。
   */
  private Set<String> inlinePropertyNames =
      new HashSet<>(Set.of("event-service-topics-subscribed"));

  /** 元数据反熵周期：定期重查版本落后的成员并重新拉取，兜底丢失的变化通知。 */
  private Duration metadataAntiEntropyInterval =
      Duration.ofMillis(DEFAULT_METADATA_ANTI_ENTROPY_INTERVAL);

  /** 元数据惰性拉取的随机抖动上限：错峰避免成员加入时的拉取风暴。 */
  private Duration metadataPullJitter = Duration.ofMillis(DEFAULT_METADATA_PULL_JITTER);
  ...
  public Set<String> getInlinePropertyNames() {
    return inlinePropertyNames;
  }

  public SwimMembershipProtocolConfig setInlinePropertyNames(
      final Set<String> inlinePropertyNames) {
    checkNotNull(inlinePropertyNames, "inlinePropertyNames cannot be null");
    this.inlinePropertyNames = new HashSet<>(inlinePropertyNames);
    return this;
  }

  public Duration getMetadataAntiEntropyInterval() {
    return metadataAntiEntropyInterval;
  }

  public SwimMembershipProtocolConfig setMetadataAntiEntropyInterval(
      final Duration metadataAntiEntropyInterval) {
    checkNotNull(metadataAntiEntropyInterval, "metadataAntiEntropyInterval cannot be null");
    checkArgument(
        !metadataAntiEntropyInterval.isNegative(),
        "metadataAntiEntropyInterval must not be negative");
    this.metadataAntiEntropyInterval = metadataAntiEntropyInterval;
    return this;
  }

  public Duration getMetadataPullJitter() {
    return metadataPullJitter;
  }

  public SwimMembershipProtocolConfig setMetadataPullJitter(final Duration metadataPullJitter) {
    checkNotNull(metadataPullJitter, "metadataPullJitter cannot be null");
    checkArgument(!metadataPullJitter.isNegative(), "metadataPullJitter must not be negative");
    this.metadataPullJitter = metadataPullJitter;
    return this;
  }
