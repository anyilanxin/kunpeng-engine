...
final class BinarySegmentDescriptorSerializer implements SegmentDescriptorSerializer {
  ...
  private static final Logger LOG =
      LoggerFactory.getLogger(BinarySegmentDescriptorSerializer.class);
  ...
  static final int ENCODING_LENGTH = Byte.BYTES + METADATA_FRAME_LENGTH + DESCRIPTOR_FRAME_LENGTH;

  // 元数据帧与描述符帧在段文件中的绝对偏移
  private static final int METADATA_FRAME_OFFSET = Byte.BYTES;
  private static final int DESCRIPTOR_FRAME_OFFSET = METADATA_FRAME_OFFSET + METADATA_FRAME_LENGTH;
  ...
  public void writeTo(final SegmentDescriptor descriptor, final ByteBuffer buffer) {
    ...
    scratchView.putLong(METADATA_FRAME_OFFSET + Integer.BYTES, descriptorChecksum, BYTE_ORDER);

    LOG.trace(
        "已写出段 {} 的描述符，起始索引 {}，校验和 {}", descriptor.id(), descriptor.index(), descriptorChecksum);
  }
  ...
  private SegmentDescriptor decodeDescriptor(final ByteBuffer buffer) {
    ...
    final long storedChecksum = view.getLong(METADATA_FRAME_OFFSET + Integer.BYTES, BYTE_ORDER);
