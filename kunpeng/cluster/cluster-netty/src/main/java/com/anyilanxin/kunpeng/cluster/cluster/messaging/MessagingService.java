...
public interface MessagingService {
  ...
  /**
   * Sends a message asynchronously to multiple communication addresses（群发）。
   *
   * <p>Aeron 实现走 MDC 多目的地发布：组帧与（启用时的）加密只做一次，由驱动扇出到全部目的地。
   *
   * <p>默认实现等价于逐地址 {@link #sendAsync(Address, String, byte[], boolean)}，供其他实现回退。
   *
   * @param addresses addresses to send the message to.
   * @param type type of message.
   * @param payload message payload bytes.
   * @return future that is completed when the message is sent
   */
  default CompletableFuture<Void> sendAsync(
    ...
    final CompletableFuture<?>[] futures =
        addresses.stream()
            .map(address -> sendAsync(address, type, payload, true))
