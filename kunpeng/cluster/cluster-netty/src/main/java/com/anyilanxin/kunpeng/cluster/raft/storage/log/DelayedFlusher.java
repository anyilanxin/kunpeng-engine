...
/**
 * 合并刷盘信号的 {@link RaftLogFlusher}：首个请求会按配置的延迟调度一次 fsync， 在它真正执行之前，后续请求全部被吸收。只要发生过写入，日志至少每个延迟周期落盘一次。
 *
...
public final class DelayedFlusher implements RaftLogFlusher {
  ...
  public void flush(final Journal journal) {
    synchronized (pendingMonitor) {
      ...
      LOGGER.trace(
          "Delaying journal flush by {}, journal ends at index {}",
          flushDelay,
  ...
  public String toString() {
    return "DelayedFlusher{scheduler="
        + scheduler
        + ", flushDelay="
        + flushDelay
        + ", pendingFlush="
