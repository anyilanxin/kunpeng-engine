...
import com.anyilanxin.kunpeng.cluster.cluster.messaging.impl.AeronUnicastService;
...
public class AtomixCluster implements BootstrapService, Managed<Void> {
  ...
  protected AtomixCluster(
    ...
    if (config.getAeronConfig().getCryptoSenderId() == null) {
      // 加密密钥按发送方派生：默认用成员 ID（未显式设置时为随机 UUID，天然唯一），用户可用配置覆盖
      config.getAeronConfig().setCryptoSenderId(config.getNodeConfig().getId().id());
    }
    if (messagingService != null) {
      ...
      // 默认路径: 两个 Aeron 服务共享同一个传输底座(驱动/客户端/代理线程)
      final AeronTransport transport =
          new AeronTransport(
              bindAddress(config), config.getMessagingConfig(), config.getAeronConfig());
      this.messagingService =
          new AeronMessagingService(
      ...
      this.unicastService =
          unicastService != null
              ? unicastService
              : new AeronUnicastService(
                  config.getNodeConfig().getAddress(), config.getAeronConfig(), transport);
    }
  ...
  private static Address bindAddress(final ClusterConfig config) {
    final Address nodeAddress = config.getNodeConfig().getAddress();
    final MessagingConfig messagingConfig = config.getMessagingConfig();
    final int port =
        messagingConfig.getPort() != null ? messagingConfig.getPort() : nodeAddress.port();
    final String host =
  ...
  /** Builds a standalone messaging service (Aeron transport). */
  ...
  /** Builds a standalone unicast service (Aeron transport). */
  protected static ManagedUnicastService buildUnicastService(
      final ClusterConfig config, final MeterRegistry registry) {
    return new AeronUnicastService(
        config.getNodeConfig().getAddress(),
        config.getAeronConfig(),
        new AeronTransport(
            bindAddress(config), config.getMessagingConfig(), config.getAeronConfig()));
