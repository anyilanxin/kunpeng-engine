...
import com.anyilanxin.kunpeng.cluster.raft.utils.ForceConfigureQuorum;
...
public final class ReconfigurationHelper {
  ...
  /**
   * 把本节点加入集群（即加入方视角的 addPeer）。容忍上次未完成的加入：先从日志重载本地配置 并恢复角色，这样对单成员集群重试（此时新节点已经参与法定人数）才可能成功。
   *
   * <pre>{@code
  ...
  /**
   * 按顺序向剩余候选节点逐个发出加入请求。传输层故障与集群层瞬态（暂无 leader、 重配进行中）通过轮换候选节点重试；全部失败后在一个选举超时后重开一轮，直到截止。
        ...
        .whenCompleteAsync(
            (response, error) -> {
              if (error != null) {
                ...
                onJoinRejected(response, candidate, remaining, allPeers, result, giveUpAt, target);
              }
            },
            raftThread);
  }
   ...
   * }</pre>
   *
   * <p>不知道 leader 时改为请求任意其他投票成员；一个都不剩则按 {@code NO_LEADER} 失败，让调用方在选举稳定后重试。
   */
  ...
  /**
   * 丢失多数派集群的强制配置变更（changePeers 等价物）。调用方在 {@code currentIndex + 1} 处本地安装新配置（绕过共识），若身为 leader
   * 先退位，再把配置推给每个新成员，由各接收方 直接应用并提交。确认采用新配置多数派（参考 SOFAJRaft Ballot 语义）：掉线或拒绝的少数派
   * 不阻塞强制变更，过半不可达才整体失败；被丢弃的成员后续在 poll/vote 阶段被拒绝。 重试必须使用同一成员集合。
   *
   * <pre>{@code
  ...
  private void installForcedConfiguration(
    ...
    final Set<RaftMember> newMembers =
        newMembersIds.entrySet().stream()
            .map(entry -> new DefaultRaftMember(entry.getKey(), entry.getValue(), Instant.now()))
    ...
    if (currentConfiguration != null && currentConfiguration.force()) {
      ...
      // 发起方立即落盘：丢多数派时 commitIndex 推不进来，不持久化则重启回滚旧配置、被删成员复活
      raftContext.getCluster().commitCurrentConfiguration();
    }
  ...
  private void awaitForcedAcknowledgements(final CompletableFuture<Void> result) {
    ...
    final var allMembers =
        configuration.newMembers().stream().map(RaftMember::memberId).collect(Collectors.toSet());
