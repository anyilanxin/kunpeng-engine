...
public enum SwimMembershipProtocolMetricsDoc implements CustomMeterDocumentation {
  ...
  MEMBERS_INCARNATION_NUMBER(
      "zeebe_smp_members_incarnation_number",
      "Member's incarnation number, useful to observe state propagation of each member information",
      Type.GAUGE),
  /** 当前成员总数 */
  MEMBER_COUNT(
      "zeebe_smp_member_count",
      "Number of members currently known by the SWIM protocol",
      Type.GAUGE),
  /** 成员加入事件次数 */
