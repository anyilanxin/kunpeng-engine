...
/**
 * 天-时持续时间类型（dayTimeDuration）的一元测试转换器：区间语法转 between，比较运算符开头转比较表达式，其余转相等判断。
 *
 * @author zxuanhong
 * @since
 */
public class DatetimeDurationTransform implements QlexpressTransformer {
  private static final Pattern COMPARISON_PATTERN = Pattern.compile("^(==|>=|<=|>|<).+$");

  /** 返回该转换器适配的值类型：dayTimeDuration。 */
  @Override
  public DmnValueType valueType() {
    return DmnValueType.DATETIME_DURATION;
  }

  /**
   * 将天-时持续时间一元测试转换为 QlExpress 表达式：优先匹配区间语法转 between；否则以比较运算符（==、&gt;=、&lt;=、
