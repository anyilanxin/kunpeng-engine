...
/**
 * 时间类型（time）的一元测试转换器：区间语法转换为 QlExpress 的 between 判断，其余直接与输入变量拼接。
 *
 * @author zxuanhong
 * @since
 */
public class TimeTransform implements QlexpressTransformer {
  /** 返回该转换器适配的值类型：time。 */
  @Override
  public DmnValueType valueType() {
    return DmnValueType.TIME;
  }

  /**
   * 将时间一元测试转换为 QlExpress 表达式：匹配区间语法时转换为 between 区间判断，否则原样拼接为比较表达式。
   *
