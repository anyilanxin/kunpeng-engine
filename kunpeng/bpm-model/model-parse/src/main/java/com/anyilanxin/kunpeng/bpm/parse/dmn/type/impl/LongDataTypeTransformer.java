...
/**
 * 将 {@link Number} 和 {@link String} 类型的值转换为 {@link LongValue}。
 *
...
public class LongDataTypeTransformer implements DmnDataTypeTransformer {
  ...
  public TypedValue transform(final Object value) throws IllegalArgumentException {
    if (value instanceof Number) {
      final long longValue = transformNumber((Number) value);
      return Variables.longValue(longValue);

    } else if (value instanceof String) {
      final long longValue = transformString((String) value);
      return Variables.longValue(longValue);
  ...
  protected long transformNumber(final Number value) {
    if (isLong(value)) {
  ...
  protected boolean isLong(final Number value) {
    final double doubleValue = value.doubleValue();
    return doubleValue == (long) doubleValue;
  }

  protected long transformString(final String value) {
