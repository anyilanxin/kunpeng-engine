...
import com.anyilanxin.kunpeng.bpm.parse.dmn.qladapter.impl.*;
import java.util.HashMap;
import java.util.Map;

/**
 * {@link SimpleUnaryTestsTransform} 的默认实现，按决策表输入的 DMN 值类型把一元测试分发给对应的
 * {@link QlexpressTransformer} 完成 QlExpress 适配。
 *
 * @author zxuanhong
 * @since
 */
public class DefaultSimpleUnaryTestsTransform implements SimpleUnaryTestsTransform {
  private final Map<DmnValueType, QlexpressTransformer> transform = new HashMap<>();
  ...
  private DefaultSimpleUnaryTestsTransform register(final QlexpressTransformer transform) {
  ...
  public String transformSimpleUnaryTests(
    ...
    if (dmnValueType != null) {
      final QlexpressTransformer testTransform = transform.get(dmnValueType);
      ...
      final QlexpressTransformer testTransform = transform.get(DmnValueType.HYPHEN);
      return testTransform.transformSimpleUnaryTests(
          dmnExpression.getExpression(), input.getInputVariable());
    }
