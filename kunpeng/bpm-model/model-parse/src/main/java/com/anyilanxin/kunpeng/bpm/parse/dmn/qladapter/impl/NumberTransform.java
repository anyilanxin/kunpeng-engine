...
/**
 * 数值类型（number）的一元测试转换器，同时承接 integer/long/double 等 DMN 数值 typeRef：比较运算符开头转比较表达式，
 * 其余转相等判断。
 *
 * @author zxuanhong
 * @since
 */
public class NumberTransform implements QlexpressTransformer {
  private static final Pattern COMPARISON_PATTERN = Pattern.compile("^(==|>=|<=|>|<).+$");

  /** 返回该转换器适配的值类型：number。 */
  @Override
  public DmnValueType valueType() {
    return DmnValueType.NUMBER;
  }

  /**
   * 将数值一元测试转换为 QlExpress 表达式：以比较运算符（==、&gt;=、&lt;=、&gt;、&lt;）开头时直接与输入变量拼接为比较
   * 表达式，否则转换为相等判断。
   *
