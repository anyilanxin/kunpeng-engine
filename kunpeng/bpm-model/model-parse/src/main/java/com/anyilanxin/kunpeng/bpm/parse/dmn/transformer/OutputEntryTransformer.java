...
public final class OutputEntryTransformer implements ModelElementTransformer<OutputEntry> {
  ...
  @Override
  public Class<OutputEntry> getType() {
    return OutputEntry.class;
  }

  /**
   * 转换输出单元格为结论表达式元素并注册到上下文。
   *
   * @param element 待转换的 OutputEntry 模型元素
   * @param context 转换上下文
   */
  @Override
  public void transform(final OutputEntry element, final TransformContext context) {
