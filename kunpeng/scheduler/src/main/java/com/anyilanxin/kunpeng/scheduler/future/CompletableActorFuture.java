...
public class CompletableActorFuture<V> implements ActorFuture<V> {
  ...
  public void complete(final V value) {
    if (STATE.compareAndSet(this, AWAITING_RESULT, COMPLETING)) {
      ...
      throw new IllegalStateException("Future 已完成，不能再次完成");
    }
  }
  ...
  public Throwable getException() {
    if (!isCompletedExceptionally()) {
      throw new IllegalStateException("Future 未以异常方式完成");
    }
  ...
  public V get(final long timeout, final TimeUnit unit)
    ...
    if (CarrierContext.current() != null) {
      if (!isDone()) {
        LOG.error("Actor 在 future 未完成时调用了 get()。actor 必须非阻塞，请改用 actor.runOnCompletion()。");
        throw new IllegalStateException(
            "Actor 在 future 未完成时调用了 get()。actor 必须非阻塞，请改用 actor.runOnCompletion()。");
      }
    } else {
      // 非 actor 线程的阻塞等待
      ...
      try {
        ...
        while (!isDone()) {
          if (remaining <= 0) {
            LOG.error("等待超时: {} {}", timeout, unit);
            throw new TimeoutException("等待超时: " + timeout + " " + unit);
          }
  ...
  public void onComplete(final BiConsumer<V, Throwable> consumer) {
    if (CarrierContext.onActorThread()) {
      ...
      LOG.warn(
          "[PotentiallyBlocking] ActorFuture#onComplete 回调未提供 executor。"
              + "这可能阻塞完成该 future 的 actor。请改用 onComplete(consumer, executor)。");
  ...
  public void close() {
    if (isDone()) {
      // ManyToOneConcurrentLinkedQueue 不支持 clear(), 逐个排空
      while (blockedCallbacks.poll() != null) {}
      setAwaitingResult();
    } else {
