...
public final class VersionUtil {
  ...
  /**
   * @return 当前版本；无法确定时返回 'development'
   */
  public static String getVersion() {
    ...
    if (foundVersion.isPresent()) {
      ...
      LOG.warn("在环境变量、版本文件或包信息中均未找到版本号，使用 '%s' 代替".formatted(VERSION_DEV));
      version = VERSION_DEV;
    }
    return version;
  }
  ...
  private static String readProperty(final String property) {
    try (final InputStream lastVersionFileStream =
      ...
      LOG.error("无法读取版本文件: {}", VERSION_PROPERTIES_PATH, e);
    }
