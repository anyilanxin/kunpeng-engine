 ...
 *
 * <p>In case of a decision with a decision table, the result contains the output entries of a
 * matched rule. Each output value is mapped to the output {@code name} attribute. If no {@code
 * name} was given then the entry key is {@code null}.
 */
public interface DmnDecisionResultEntries extends Map<String, Object>, Serializable {
  ...
  /**
   * Returns the value of the single entry of the decision result. Asserts that the decision result
   * only has one entry.
   *
   * @param <T> the type of the result entry
   * @return the value of the single result entry or null if none exists
   * @throws DmnEngineException if more than one result entry exists
   * @see #getSingleEntryTyped()
   */
  <T> T getSingleEntry();

  /**
   * Returns the typed value of the single entry of the decision result. Asserts that the decision
   * result only has one entry.
   *
   * @param <T> the type of the result entry
