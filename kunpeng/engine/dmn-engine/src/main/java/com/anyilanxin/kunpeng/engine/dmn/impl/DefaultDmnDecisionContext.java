...
import com.anyilanxin.kunpeng.engine.dmn.DmnDecisionLogic;
...
import com.anyilanxin.kunpeng.engine.dmn.impl.delegate.DmnDecisionEvaluationEventImpl;
import com.anyilanxin.kunpeng.engine.dmn.impl.evaluation.DecisionLiteralExpressionEvaluationHandler;
import com.anyilanxin.kunpeng.engine.dmn.impl.evaluation.DecisionTableEvaluationHandler;
import com.anyilanxin.kunpeng.engine.dmn.impl.evaluation.DmnDecisionLogicEvaluationHandler;
import com.anyilanxin.kunpeng.engine.dmn.impl.hitpolicy.HitPolicyEntry;
...
public class DefaultDmnDecisionContext {
  ...
  public DefaultDmnDecisionContext(final DefaultDmnEngineConfiguration configuration) {
  ...
  public DmnDecisionResult evaluateDecision(final DmnDecision decision, final VariableContext variableContext) {
    ...
    final VariableMap variableMap = buildVariableMapFromVariableContext(variableContext);

    final List<DmnDecision> requiredDecisions = new ArrayList<DmnDecision>();
    ...
    for (final DmnDecision evaluateDecision : requiredDecisions) {
      final DmnDecisionLogicEvaluationHandler handler = getDecisionEvaluationHandler(evaluateDecision);
  ...
  protected VariableMap buildVariableMapFromVariableContext(final VariableContext variableContext) {

    final VariableMap variableMap = Variables.createVariables();

    final Set<String> variables = variableContext.keySet();
  ...
  protected void buildDecisionTree(final DmnDecision decision, final List<DmnDecision> requiredDecisions) {
  ...
  protected DmnDecisionLogicEvaluationHandler getDecisionEvaluationHandler(final DmnDecision decision) {
  ...
  protected void addResultToVariableContext(
    final DmnDecisionResult evaluatedResult, final VariableMap variableMap, final DmnDecision evaluatedDecision) {
  ...
  protected boolean isDecisionTableWithCollectOrRuleOrderHitPolicy(final DmnDecision evaluatedDecision) {
  ...
  protected void generateDecisionEvaluationEvent(
    ...
    final DmnDecisionEvaluationEventImpl decisionEvaluationEvent = new DmnDecisionEvaluationEventImpl();
    ...
    for (final DmnDecisionEvaluationListener evaluationListener : evaluationListeners) {
