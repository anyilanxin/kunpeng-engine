...
import com.anyilanxin.kunpeng.protocol.business.impl.eventlog.UnwrittenRecord;
...
public class BatchProcessingCollect implements CloseableSilently {
  private final Deque<BusinessLogRecord<?>> toProcess = new ArrayDeque<>();
  ...
  public BatchProcessingCollect(
      ...
      final BusinessRepositoryAppliers applier,
  ...
  public BatchProcessingCollect addInitCommand(final BusinessLogRecord<?> logRecord) {
    reset();
    processSize++;
  ...
  public BatchProcessingCollect addCommand(
      final long key, final UnifiedRecordValue recordValue, final RecordMetadata metadata) {
  ...
  public BatchProcessingCollect addCommandWrite(
      final UnifiedRecordValue recordValue, final RecordMetadata metadata) {
  ...
  public BatchProcessingCollect adResponse(
    ...
    final BrokerResponseWriter<UnifiedRecordValue> responseWriter =
        commandApiHandle.newResponse(lifeCycle, requestId, partitionId);
  ...
  public BatchProcessingCollect adErrorResponse(
    ...
    final BrokerResponseWriter<UnifiedRecordValue> responseWriter =
        commandApiHandle.newResponse(null, requestId, partitionId);
  ...
  public BusinessLogRecord<?> next() {
    final BusinessLogRecord<?> logRecord = toProcess.removeFirst();
    final RecordMetadata copyMetadata = logRecord.getMetadata().copy();
  ...
  public List<AppendEntry> waitWrite() {
    if (!toProcess.isEmpty()) {
      for (final BusinessLogRecord<?> logRecord : toProcess) {
        final AppendEntry logAppendEntry =
  ...
  public List<BusinessLogRecord<?>> waitProcess() {
