 ...
 */
package com.anyilanxin.kunpeng.engine.bpmn.commandapi.incident;
...
/**
 * 事件（incident）API 处理器抽象基类。
 *
 * @author zxuanhong
 * @since 2026.9.0
 */
public abstract class IncidentApiAbstractProcessor
    implements LogEventProcessor<IncidentResolveRequestRecord> {
  protected final LogEventWriter writer;
  protected final ImmutableJobRepository job;
  protected final ImmutableIncidentRepository incident;
  ...
  public IncidentApiAbstractProcessor(final LogEventWriter writer) {
    this.writer = writer;
  ...
  @Override
  public CommandApiIncidentValueLifeCycle[] valueLifeCycles() {
    return new CommandApiIncidentValueLifeCycle[] {processState()};
  }

  public abstract CommandApiIncidentValueLifeCycle processState();
