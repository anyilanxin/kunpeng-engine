...
public interface RegistryMetrics {
  ...
  IntConsumer forLifeCycle(ValueLifeCycle lifeCycle);

  /** 将每个 lifeCycle 的集合大小以 gauge 上报，并以 lifeCycle 的类名与名称打标签。 */
  class BoundedRegistryMetrics implements RegistryMetrics {

    private final Map<ValueLifeCycle, AtomicInteger> sizes = new HashMap<>();
    ...
    @Override
    public IntConsumer forLifeCycle(final ValueLifeCycle lifeCycle) {
      return sizes.computeIfAbsent(lifeCycle, this::registerSizeReporter)::set;
    }

    private AtomicInteger registerSizeReporter(final ValueLifeCycle lifeCycle) {
