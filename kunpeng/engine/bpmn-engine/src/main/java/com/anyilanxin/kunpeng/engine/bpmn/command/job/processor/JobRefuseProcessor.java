 ...
 * @since 2026.9.0
 */
public class JobRefuseProcessor extends JobAbstractProcessor {

  public JobRefuseProcessor(final LogEventWriter writer) {
    super(writer);
  }

  @Override
  public JobLifeCycle processState() {
    return JobLifeCycle.REFUSING;
  }
  ...
  public void processRecord(final BusinessLogRecord<JobRecord> logRecord) {
    ...
    value.setState(JobState.TERMINATED);
    value.setLifeCycle(JobLifeCycle.REFUSED);
    writer.addEvent(value.getJobId(), JobLifeCycle.REFUSED, logRecord.getRequestId(), value);
    ...
    switch (jobKind) {
      ...
      case USER_TASK_LISTENER -> {
        final UserTaskRecord record = userTask.getRecord(value.getTaskId());
        writer.addCommand(
            record.getTaskId(), UserTaskLifeCycle.LISTENER_DENY, logRecord.getRequestId(), record);
      }
    }
  }
}
