...
/**
 * 按 {@link ValueLifeCycle lifeCycle} 分组去重在途命令 key。
 *
...
public interface PendingCommandRegistry {
  ...
  void add(ValueLifeCycle lifeCycle, long key);

  /** 返回 {@code lifeCycle} 下 {@code key} 的命令是否已在途。 */
  boolean contains(ValueLifeCycle lifeCycle, long key);

  /** 遗忘 {@code lifeCycle} 下 {@code key} 的在途命令。 */
  void remove(ValueLifeCycle lifeCycle, long key);
