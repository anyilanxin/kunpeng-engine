 ...
 */
package com.anyilanxin.kunpeng.engine.bpmn.scheduling.cache;

import com.anyilanxin.kunpeng.engine.bpmn.scheduling.PendingCommandRegistry;
...
/**
 * 按 {@link ValueLifeCycle lifeCycle} 分键、容量有界的线程安全 {@link PendingCommandRegistry}。
 *
...
public final class BoundedPendingCommandRegistry implements PendingCommandRegistry.Stageable {
  ...
  private final Map<ValueLifeCycle, BoundedKeySet> sets;
   ...
   */
  public BoundedPendingCommandRegistry(final Map<ValueLifeCycle, BoundedKeySet> sets) {
  ...
  public static BoundedPendingCommandRegistry forIntents(
      final RegistryMetrics metrics, final ValueLifeCycle... lifeCycles) {
    final Map<ValueLifeCycle, BoundedKeySet> built = new HashMap<>();
    for (final ValueLifeCycle lifeCycle : lifeCycles) {
  ...
  @Override
  public void add(final ValueLifeCycle lifeCycle, final long key) {
  ...
  @Override
  public boolean contains(final ValueLifeCycle lifeCycle, final long key) {
    final var set = sets.get(lifeCycle);
    return set != null && set.contains(key);
  }

  @Override
  public void remove(final ValueLifeCycle lifeCycle, final long key) {
    final var set = sets.get(lifeCycle);
    if (set != null) {
      set.remove(key);
    }
  ...
  private final class StagedRegistry implements Staging {
    ...
    private final Map<ValueLifeCycle, LongHashSet> buffered = new HashMap<>();
    ...
    private LongHashSet buffer(final ValueLifeCycle lifeCycle) {
