 ...
 * @since 2026.9.0
 */
public class BatchTerminateExecutorProcessor extends AbstractBatchProcessor {
  ...
  public BatchTerminateExecutorProcessor(final LogEventWriter writer) {
    super(writer);
  ...
  public ProcessInstanceBatchState valueLifeCycle() {
    return ProcessInstanceBatchState.ACTIVATE_TERMINATE;
  }
  ...
  public void processRecord(final BusinessLogRecord<ProcessInstanceBatchRecord> record) {
    ...
    if (!activityInstance.haveChildRecord(value.getBatchBusinessId())) {
      writer.addEvent(
          record.getKey(), ProcessInstanceBatchState.BATCH_COMPLETED, record.getRequestId(), value);
