 ...
 * @since 2026.9.0
 */
public class BpmnUserTaskCompletingProcessor extends BpmnUserTaskElementAbstractProcessor {

  public BpmnUserTaskCompletingProcessor(final LogEventWriter writer) {
    super(writer);
  }
  ...
  public void process(
    ...
    final UserTaskRecord value = logRecord.getValue();
    value.setState(UserTaskState.COMPLETING);
    value.setLifeCycle(UserTaskLifeCycle.COMPLETING);
    writer.addEvent(
        value.getTaskId(), UserTaskLifeCycle.COMPLETING, logRecord.getRequestId(), value);

    value.setListenerType(UserTaskListenerType.COMPLETE);
    writer.addCommand(
        value.getTaskId(), UserTaskLifeCycle.LISTENER_CREATE, logRecord.getRequestId(), value);
  }
}
