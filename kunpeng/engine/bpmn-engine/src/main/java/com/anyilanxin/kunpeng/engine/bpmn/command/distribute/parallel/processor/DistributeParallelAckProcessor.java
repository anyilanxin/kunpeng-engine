 ...
 * @since 2026.9.0
 */
public class DistributeParallelAckProcessor extends AbstractDistributeParallelProcessor {
  private static final Logger LOG = LoggerFactory.getLogger(DistributeParallelAckProcessor.class);
  final LogEventWriter writer;
  private final ImmutableDistributeParallelRepository distribute;

  public DistributeParallelAckProcessor(final LogEventWriter writer) {
    this.writer = writer;
  ...
  public void processRecord(final BusinessLogRecord<DistributeParallelRecord> record) {
    ...
    LOG.debug("distributeAckProcessor fromSourceId:{}", value.getCurrentDistributeIndex());
    writer.addEvent(
        value.getDistributeId(),
        DistributeParallelLifeCycle.DISTRIBUTE_ACKED,
        record.getRequestId(),
        distributeRecord);

    LOG.debug("haveDistributeAck: {}", distribute.haveDistributeAck(value.getDistributeId()));
    if (!distribute.haveDistributeAck(value.getDistributeId())) {
      if (distributeRecord.isHaveFollowUp()) {
        writer.addCommand(
  ...
  public DistributeParallelLifeCycle valueLifeCycle() {
    return DistributeParallelLifeCycle.DISTRIBUTE_ACK;
  }
}
