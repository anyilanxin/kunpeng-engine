...
`ScheduledTimer`（cancel）、`ActorCondition`（signal/cancel）、`ActorClock`（current()）、`AsyncClosable`/`CloseableSilently`、retry 5 策略、exception 4 类、`AdminRepositoryLoggers`（命名 logger 常量）。
