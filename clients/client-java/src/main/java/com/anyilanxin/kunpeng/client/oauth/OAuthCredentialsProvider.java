...
public final class OAuthCredentialsProvider implements CredentialsProvider {
  ...
  private static final ObjectReader CREDENTIALS_READER =
      JSON_MAPPER.readerFor(KunpengClientCredentials.class);
  ...
  public void applyCredentials(final CredentialsApplier applier) throws IOException {
    final KunpengClientCredentials kunpengClientCredentials =
        credentialsCache.computeIfMissingOrInvalid(clientId, this::fetchCredentials);

    String type = kunpengClientCredentials.getTokenType();
    ...
    applier.put(
        HEADER_AUTH_KEY, String.format("%s %s", type, kunpengClientCredentials.getAccessToken()));
  }
      ...
      return statusCode.isUnauthorized()
          && credentialsCache
              .withCache(
                  clientId,
                  value -> {
                    final KunpengClientCredentials fetchedCredentials = fetchCredentials();
  ...
  private KunpengClientCredentials fetchCredentials() throws IOException {
    ...
    connection.setRequestProperty("User-Agent", "kunpeng-client-java/" + VersionUtil.getVersion());
    ...
    try (final InputStream in = connection.getInputStream();
      ...
      final KunpengClientCredentials fetchedCredentials = CREDENTIALS_READER.readValue(reader);
