...
public interface CommandWithTenantStep<T> {
   ...
   * kunpeng cluster. After enabling multi-tenancy, these entities can still be interacted by using
   ...
   * <h1>Multi-tenancy</h1>
   *
   * <p>Multiple tenants can share a kunpeng cluster. Entities can be assigned to a specific tenant
   * using an identifier. Only that tenant can access these entities.
   *
   * <p>Any entities created before multi-tenancy has been enabled in the kunpeng cluster, are
