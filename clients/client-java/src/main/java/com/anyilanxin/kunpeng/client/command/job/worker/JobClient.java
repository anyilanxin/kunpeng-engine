...
public interface JobClient {
   ...
   *
   * <pre>{@code
   * final Consumer<IncidentResolveCommand> consumer = ...; // do something with the consumed job
   * final KunpengFuture<StreamJobsResponse> stream = jobClient
   ...
   *
   * <ul>
   *   <li>Closing the kunpeng client
   *   <li>Cancelling the result of {@link StreamJobsCommandStep3#send()} via {@link
   *       KunpengFuture#cancel(boolean)} (the argument is irrelevant)
   ...
   * </ul>
   *
   * <p>NOTE: streams can be closed for various reasons - for example, the server is restarting. As
