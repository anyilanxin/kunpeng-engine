...
import static SebGeneratorUtil.*
...
class SbeGeneratorPlugin implements Plugin<Project> {
    ...
    void apply(Project project) {
        ...
        project.afterEvaluate {
            ...
            input = tree(project, ext.src).files.absolutePath.toSet() as Set<File>
    ...
    void validateTask(Project project) {
        project.tasks.register(VALIDATE_TASK, DefaultTask) {
            group = PLUGIN_GROUP
    ...
    void genJavaTask(Project project, SbeGeneratorPluginExtension ext) {
        project.tasks.register(GENERATE_JAVA_TASK, JavaExec) {
            group = PLUGIN_GROUP
    ...
    void compileJavaTask(Project project, SbeGeneratorPluginExtension ext) {
        project.tasks.register(COMPILE_JAVA_TASK, JavaCompile) {
    ...
    void packJavaTask(Project project, SbeGeneratorPluginExtension ext) {
        project.tasks.register(PACK_JAVA_TASK, Jar) {
    ...
    void genCppTask(Project project, SbeGeneratorPluginExtension ext) {
        project.tasks.register(GENERATE_CPP_TASK, JavaExec) {
            group = PLUGIN_GROUP
    ...
    void cmakeCppTask(Project project, SbeGeneratorPluginExtension ext) {
        project.tasks.register(CMAKE_CPP_TASK, DefaultTask) {
    ...
    void packCppTask(Project project, SbeGeneratorPluginExtension ext) {
        project.tasks.register(PACK_CPP_TASK, Tar) {
            dependsOn(CMAKE_CPP_TASK)
            group = PLUGIN_GROUP
